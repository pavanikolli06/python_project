...
class Student:
    def __init__(self, main):
        ...
        self.tree = ttk.Treeview(self.Frame_2, column=("c1","c2","c3","c4","c5","c6","c7","c8"), show='headings',height=25)
    ...
    def Delete(self):  
        ...
        selected_item = self.tree.item(item)['values'][0]  # Corrected to self.tree
        print(selected_item)
        c = sqlite3.connect("Student.db")
        cursor = c.cursor()
        cursor.execute("DELETE FROM Student WHERE ID=?", (selected_item,))  # Corrected SQL syntax
        print("Value Deleted")
        c.commit()
        c.close()
    ...
    def Update(self):  
        ...
        c = sqlite3.connect("Student.db")
        cursor = c.cursor()
        cursor.execute("UPDATE Student SET ID=?, NAME=?, AGE=?, DOB=?, GENDER=?, CITY=? ,GRADE=? ,ATTENDENCE=? WHERE ID=?", (id, name, age, dob, gender, city, selected_item))
        c.commit()
        c.close()
    ...
    def clear(self):
        ...
        # Assuming you have entry fields defined as self.Id_Entry, self.Name_Entry, etc.
